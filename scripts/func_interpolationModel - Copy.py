...
def readBasis(basis,filename):
    '''
    Read basis in filename in HDF5 format
    '''
    f1 = h5py.File(filename, 'r')
    for icol in range(basis.shape[1]):
        basis[:,icol] = np.array(f1.get(f'basis/{icol}'))
    return basis

def readMean(mean,filename):
    '''
    Read mean for snpashots in filename in HDF5 format
    '''
    f1 = h5py.File(filename, 'r')
    mean = np.array(f1.get(f'mean'))
    return mean

def readEigenValues(sigma,filename):
    '''
...
def trainModel(case):
    ...
    results_folder = case.results_folder
    root_data = case.root_data  
    ...
    for filename in glob.glob(fr'{results_folder}\*_angle_*.vtk'):
       os.remove(filename)  
    for filename in glob.glob(fr'{results_folder}\basis'):
       os.remove(filename)
    ...
    if use_SVD:    
        ...
        # wrting basis, mean and sigular values
        file_basis = fr'{root_data}\basis'
        ...
        u_aux = np.copy(u)
        u_aux[:,:] = 0.0 
        mean_aux = np.zeros([data_training_mean.shape[0]])
        s_aux = np.zeros([s.shape[0]])
        u_aux = readBasis(u_aux, file_basis)
        mean_aux = readMean(mean_aux, filename)
        s_aux = readEigenValues(s_aux, filename)
        diff_basis = np.linalg.norm(np.subtract(u_aux,u),2)
        diff_mean = np.linalg.norm(np.subtract(mean_aux,data_training_mean),2)
        diff_s = np.linalg.norm(np.subtract(s_aux,s),2)
